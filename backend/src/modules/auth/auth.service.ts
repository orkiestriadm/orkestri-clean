...
const ALL_PERMISSIONS: { recurso: string; acao: string; descricao: string }[] = [
  ...
  // Processos e Capacidade ficam no Quality. Usavam `projetos:ver`, e dar acesso
  // "só a Projetos" trazia o Quality junto; agora cada um tem a sua.
  { recurso: "processos",     acao: "ver",         descricao: "Ver modelos de processo (Quality)" },
  { recurso: "processos",     acao: "editar",      descricao: "Criar, editar e excluir modelos de processo" },
  { recurso: "capacidade",    acao: "ver",         descricao: "Ver capacidade e carga de trabalho da equipe" },
...
const ROLE_DEFAULTS: Record<string, { nivel: number; descricao: string; permissoes: string[] }> = {
  ...
  gestor: {
    ...
    permissoes: [
      ...
      "processos:ver","processos:editar","capacidade:ver",
...
export class AuthService implements OnModuleInit {
  ...
  private validatePasswordStrength(senha: string) {
    ...
    if (!/[^A-Za-z0-9]/.test(senha)) throw new BadRequestException("A senha deve conter ao menos um caractere especial (!@#$%^&* etc).");
  }
}
