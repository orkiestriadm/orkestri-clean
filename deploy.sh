...
echo ""
echo "Próximo passo — acesse o servidor e execute:"
echo "  ssh $SSH_OPTS $SERVER_USER@$SERVER_IP"
echo ""
echo "  # Primeira vez (instala Docker, sobe stack):"
echo "  bash $REMOTE_DIR/scripts/setup-server.sh"
echo ""
echo "  # Após DNS propagar (obtém cert SSL):"
echo "  cd $REMOTE_DIR && bash scripts/init-ssl.sh"
echo ""
echo "  # Deploy subsequente (rebuild + restart):"
echo "  cd $REMOTE_DIR && docker compose -f docker-compose.yml -f docker-compose.prod.yml up -d --build"
