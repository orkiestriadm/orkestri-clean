  ...
  return (
    <div className="w-full max-w-3xl">
      <div className="flex items-end justify-between gap-6">
        <div>
          <p className="mb-2 flex items-center gap-2 text-[12px] font-semibold uppercase tracking-[0.14em] text-marca-clara [@media(max-height:960px)]:hidden">
            <Newspaper className="h-4 w-4" aria-hidden /> Acontece na {MARCA.replace(/^HUB\s+/i, "")}
          </p>
          <h2 className="text-[2rem] font-bold leading-tight tracking-[-0.03em] [@media(min-height:961px)]:xl:text-[2.3rem]">
      ...
      <ul className="mt-5 grid grid-cols-2 gap-4">
        {itens.map((n) => (
          <li key={n.link}>
            ...
            >
              <div className="relative h-[clamp(7rem,16vh,11rem)] overflow-hidden bg-white/[0.04] [@media(max-height:780px)]:hidden">
                {n.imagem && (
                  <img
                    src={n.imagem}
                    alt=""
                ...
                <div aria-hidden className="absolute inset-x-0 bottom-0 h-1/3 bg-gradient-to-t from-[#08090c]/50 to-transparent" />
              </div>
              <div className="flex flex-1 flex-col px-4 py-3.5">
                <p className="line-clamp-3 text-[14.5px] font-semibold leading-snug text-white">{n.titulo}</p>
                {n.resumo && (
                ...
                <div className="mt-auto flex items-center justify-between gap-3 pt-3">
                  <span className="text-[11.5px] font-medium tabular-nums text-white/45">{dataCurta(n.data)}</span>
                  <span className="inline-flex items-center gap-1 text-[12px] font-semibold uppercase tracking-wide text-marca-clara">
