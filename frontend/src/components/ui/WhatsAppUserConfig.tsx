...
export default function WhatsAppUserConfig() {
  ...
  useEffect(() => {
    api.get("/users/me/whatsapp")
      .then(r => { setNumero(r.data.whatsapp || ""); setAtivo(r.data.whatsappAlertas || false); })
      .catch(() => {})
      ...
      <div className="card" style={{ padding:"20px 24px" }}>
        <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:20, paddingBottom:12, borderBottom:"1px solid var(--border-subtle)" }}>
          ...
          <div>
            ...
            <div style={{ fontSize:11, color:"var(--text-muted)", marginTop:2 }}>Receba lembretes de eventos no seu WhatsApp</div>
          </div>
        </div>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"12px 16px", borderRadius:10, background:"var(--bg-hover)", border:"1px solid var(--border-subtle)" }}>
            <div>
              <div style={{ fontSize:13, fontWeight:500, color:"var(--text-primary)" }}>Receber alertas de eventos</div>
              <div style={{ fontSize:11, color:"var(--text-muted)", marginTop:2 }}>60 min, 15 min, 5 min e na hora do evento</div>
            </div>
          ...
          <div style={{ background:"var(--bg-hover)", borderRadius:10, padding:"12px 16px" }}>
            <div style={{ fontSize:11, color:"var(--text-muted)", fontFamily:"var(--font-mono)", letterSpacing:"0.08em", marginBottom:10 }}>ALERTAS QUE VOCE RECEBERA</div>
            <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
              {[
                { tempo:"60 min antes", desc:"Lembrete antecipado",      color:"var(--accent-violet)" },
                { tempo:"15 min antes", desc:"Prepare-se para o evento",  color:"var(--accent-amber)"  },
                { tempo:"5 min antes",  desc:"Urgente - evento proximo",  color:"var(--accent-red)"    },
                { tempo:"Na hora",      desc:"Evento acontecendo agora",  color:"var(--accent-red)"    },
              ].map((a, i) => (
