#!/bin/bash
# Importacao de planilha OPEX no modulo Orcamento: botao "Importar OPEX" cria ciclos
# (2025/2026), categorias, itens e valores mensais (previsto v2 + realizado).
# Backend (endpoint importar-opex) + frontend (botao + parser).
set -e
SERVER="planner@10.192.4.123"
cd "$(dirname "$0")"

echo "==> Deploy IMPORTACAO OPEX -> producao $SERVER"
